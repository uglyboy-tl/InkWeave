...
| **ChoiceComponents** | JS → React | React 组件 | 注册自定义选项组件 | `@inkweave/react` |
...
#### ChoiceParser vs ChoiceComponents

| 系统 | 职责 | 包 |
|------|------|-----|
| **ChoiceParser** | 解析 choice 标签，修改 `choice.type`、`choice.val` | `@inkweave/core` |
| **ChoiceComponents** | 注册和渲染自定义 React 组件 | `@inkweave/react` |
...
// plugins 包 - 注册组件
ChoiceComponents.register('auto', AutoButtonComponent);
```
...
| 模块化 | 全局加载 | 按需加载 |
| 事件系统 | 有 | 暂无 |
| 选项扩展 | 无 | ChoiceParser + ChoiceComponents |
| 包结构 | 单文件 | Monorepo 多包 |
...
| autoButton | ChoiceParser + ChoiceComponents | `* [auto:5]` | 自动选择按钮 |
| cdButton | ChoiceParser + ChoiceComponents | `* [cd:3]` | 冷却按钮 |
